...
def compute_energy(lattice, L=L, N_a=N_a, cutoff=cutoff,cyclic=cyclic,chemical_potential=chemical_potential):
    E = 0
    a = L / (N_a - 1)
...
if __name__ == "__main__":
    if uv:
        file = open(f"./Results/2DLJ_occ_L={L}_N_a={N_a}_cutoff_results.txt", "a")
    else:
        file = open(f"./Results/2DLJ_occ_L={L}_N_a={N_a}_results_pbc.txt", "a")
        ...
        while not terminate:
            ...
            for i in range(N_a):
                for j in range(N_a):
                    ...
                    dE += -chemical_potential * (1 - 2 * lattice[i, j])
